...
namespace SampleProject.API.Orders.ChangeCustomerOrder
{
    public class ChangeCustomerOrderCommandHandler : IRequestHandler<ChangeCustomerOrderCommand>
    {
        ...
        public ChangeCustomerOrderCommandHandler(ICustomerRepository customerRepository, IProductRepository productRepository, IForeignExchange foreignExchange)
        {
        ...
        public async Task<Unit> Handle(ChangeCustomerOrderCommand request, CancellationToken cancellationToken)
        {
            ...
            var existingProducts = await this._productRepository.GetByIdsAsync(existingProductsIds);

            var selectedProducts =
                await this._productRepository.GetByIdsAsync(request.Products.Select(x => new ProductId(x.Id)).ToList());
